...
class PlantsProvider {
  ...
  Future getAllPlants() async {
    ...
    String url = routes.urlBase + routes.listarPlants + '?porPagina=100';
    ...
    try {
      ...
      RxVariables.errorMessage = e.response!.data["message"]
          ...
          .replaceAll("}", "")
          .replaceAll("]", "");
      rxVariables.gvBeSubListPlants.sink.addError(
  ...
  Future createPlant(String nombrePlanta, int idCatPais) async {
    ...
    try {
      final dio = Dio();
      final data = {'nombre_planta': nombrePlanta, 'id_cat_pais': idCatPais};
      final resp = await dio.post(url, data: data, options: headerWithToken);
      await getAllPlants();
      return resp.data;
    } on DioError catch (e) {
